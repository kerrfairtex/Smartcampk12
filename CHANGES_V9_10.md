# CHANGES for versions 9 and 10
## KerrFairtex Student Information System

Changes in 10.9.8
-----------------
- Fix remove "-master" suffix from add-on folder in Modules.inc.php & Plugins.inc.php
- Update Default School Year to 2023 in config.inc.sample.php, kerrfairtex.sql & kerrfairtex_mysql.sql
...
- SQL gradebook_assignments table: Add WEIGHT column in Update.fnc.php, kerrfairtex.sql & kerrfairtex_mysql.sql
...
- SQL set N/A grade GPA to NULL in kerrfairtex.sql, kerrfairtex_es.sql, kerrfairtex_fr.sql & kerrfairtex_mysql.sql
- Add Portuguese (Brazil) translation in InstallDatabase.php, REFERENCE.md, locale/pt_BR.utf8/ & kerrfairtex_pt_BR.sql, thanks to Emerson Barros
...
- Fix MySQL 8 syntax error, 'rank' is a reserved keyword in kerrfairtex_mysql.sql
...
- MySQL + MariaDB < 10.5 fix FOREIGN KEY constraint syntax in kerrfairtex_mysql.sql
...
- Spanish translation add ¿ character before questions in kerrfairtex.po
- HTML add "Check All" checkbox after Periods in AddAbsences.php
- Truncate Assignment title to 36 chars in AnomalousGrades.php
- SQL resources table: Add PUBLISHED_PROFILES & PUBLISHED_GRADE_LEVELS columns in Update.fnc.php, kerrfairtex.sql & kerrfairtex_mysql.sql
...
- Corrections for French translation in kerrfairtex.po
...
Changes in 10.6.1
-----------------
- PostgreSQL Fix Class Rank float comparison issue: do NOT use double precision type (inexact), use numeric (exact) in kerrfairtex.sql, thanks to @fatahou
- MySQL Fix Class Rank float comparison issue: do NOT use double precision type (inexact), use numeric(22,16) (exact) in kerrfairtex_mysql.sql, thanks to @fatahou
...
Changes in 10.5.1
-----------------
- Fix MySQL error Unknown system variable 'storage_engine' in InstallDatabase.php, Modules.inc.php, Plugins.inc.php & kerrfairtex_mysql.sql
...
Changes in 10.4.3
-----------------
- MySQL always use InnoDB (default), avoid MyISAM in InstallDatabase.php, Modules.inc.php, Plugins.inc.php & kerrfairtex_mysql.sql
...
- Translate "Enroll student for next school year" to French & Spanish in kerrfairtex.po
...
- Add dual VIEW for compatibility with MySQL 5.6 to avoid syntax error when WHERE without FROM clause in kerrfairtex.sql & Update.fnc.php
...
- Fix French translation for "Waiver" & "Refund" in kerrfairtex.po
...
- SQL change modname column type from text to varchar(150) to match with MySQL key index limitation in kerrfairtex.sql
- SQL change program column type from text to varchar(100) NOT NULL to match with MySQL index limitation in kerrfairtex.sql
- SQL change schools column type from text to varchar(150) to match with MySQL index limitation in kerrfairtex.sql
...
- SQL remove use of nextval in kerrfairtex_fr.sql
...
- Fix typo "inexistant" to "nonexistent" & update translations in Modules.inc.php, Plugins.inc.php & kerrfairtex.po
...
- SQL change amount columns type from numeric to numeric(14,2) NOT NULL in kerrfairtex.sql & StudentFees.php
- SQL change minutes,minutes_present,points,default_points,length,count_weighted_factors,count_unweighted_factors columns type from numeric to integer in kerrfairtex.sql, UpdateAttendanceDaily.fnc.php, Assignments.php, MassCreateAssignments.php & Periods.php
- SQL change gp & gpa columns type from numeric to numeric(7,2) in kerrfairtex.sql
- SQL change sum/cum factors & credit_attempted/earned columns type from numeric to double precision in kerrfairtex.sql
...
- SQL fix CREATE INDEX on right table in kerrfairtex.sql
- SQL remove unused indices for various tables in kerrfairtex.sql
- SQL match index with FOREIGN KEY for various tables in kerrfairtex.sql
...
- SQL start staff_fields ID sequence at 200000000 for coherence with custom_fields in kerrfairtex.sql & Fields.fnc.php
...
- Add MySQL database dump in kerrfairtex_mysql.sql
- Log "KerrFairtex HACKING ATTEMPT" into Apache error.log in HackingLog.fnc.php
...
- SQL use timestamp type: standard & without time zone by default in kerrfairtex.sql
...
- SQL set default nextval (auto increment) for KerrFairtex version < 5.0 on install & old add-ons in Update.fnc.php
- SQL no more cast MARKING_PERIOD_ID column as text/varchar in kerrfairtex.sql & InputFinalGrades.php
- PLpgSQL compact & consistent function declaration in kerrfairtex.sql
...
- SQL rename character varying & character data types to varchar & char in kerrfairtex.sql
- SQL replace use of STRPOS() with LIKE, compatible with MySQL in PortalPollNotes.fnc.php & Courses.php
- SQL fix French & Spanish translation for Create Parent Users & Notifiy Parents email templates in kerrfairtex_fr.sql & kerrfairtex_es.sql
...
- SQL student_report_card_grades table: convert MARKING_PERIOD_ID column to integer in Update.fnc.php, kerrfairtex.sql, EditReportCardGrades.php, FinalGrades.php & ReportCards.fnc.php
...
- Add decimal & thousands separator configuration in Help_en.php, Currency.fnc.php, Configuration.php, kerrfairtex.sql & kerrfairtex_fr.sql
...
- Update French & Spanish translation in kerrfairtex.po & help.po
- Update Default School Year to 2022 in config.inc.sample.php & kerrfairtex.sql
...
- Fix French translation for "Not due" in kerrfairtex.po
...
- Add Secure KerrFairtex link in INSTALL.md
...
- Add translations for Calendar days legend in kerrfairtex.po
...
- French translation: remove capitalization & use articles in kerrfairtex.po, help.po & kerrfairtex_fr.sql
...
- SQL set default FAILED_LOGIN_LIMIT to 30 in kerrfairtex.sql, thanks to @domiee13
...
- Remove unused index ON attendance_period (attendance_code) & ON student_report_card_grades (school_id) in kerrfairtex.sql & kerrfairtex_mysql.sql
