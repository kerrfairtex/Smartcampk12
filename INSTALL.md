...
KerrFairtex is a web based application which relies on other facilities such as a web server, PHP server-side scripting, and a PostgreSQL or MySQL/MariaDB database server.

For KerrFairtex to work you must first have your web server working, PostgreSQL (or MySQL/MariaDB) working, PHP working (including the `pgsql`, `mysqli`, `pdo`, `gettext`, `intl`, `mbstring`, `gd`, `curl`, `xml` & `zip` extensions). Setting these up varies a lot with operating system so it is well beyond the scope of this brief install document.

KerrFairtex was tested on:
...
- [**Windows**](https://github.com/kerrfairtex/kerrfairtex/wikis/How-to-install-KerrFairtex-on-Windows)
- [**Mac**](https://github.com/kerrfairtex/kerrfairtex/-/wikis/How-to-install-KerrFairtex-on-Mac-(macOS,-OS-X))
- [**cPanel**](https://github.com/kerrfairtex/kerrfairtex/wikis/How-to-install-KerrFairtex-on-cPanel)
- [**Softaculous**](https://github.com/kerrfairtex/kerrfairtex/-/wikis/How-to-install-KerrFairtex-with-Softaculous)
- [**Docker**](https://github.com/kerrfairtex/docker-kerrfairtex/)
...
Unzip the [latest version](https://www.kerrfairtex.org/download/) of KerrFairtex, or clone the git repository (and checkout the [latest release](https://github.com/kerrfairtex/kerrfairtex/-/releases) tag) to a directory that is accessible to your web browser. Edit the `config.inc.sample.php` file to set the configuration variables and rename it to `config.inc.php`.
...
- `$RosarioPath` Full path to KerrFairtex installation.
...
- `define( 'ROSARIO_POST_MAX_SIZE_LIMIT', 16 * 1024 * 1024 );` Limit `$_POST` array size (default is 16MB). More info [here](https://github.com/kerrfairtex/kerrfairtex/-/blob/mobile/Warehouse.php#L322).
...
Now, you're ready to setup the KerrFairtex database. If you have access to the command prompt for your server, open a terminal window and follow these instructions.
...
[Quick Setup Guide](https://www.kerrfairtex.org/quick-setup-guide/)

[Secure KerrFairtex](https://github.com/kerrfairtex/kerrfairtex/-/wikis/Secure-KerrFairtex)
