...
- [**Windows**](https://github.com/kerrfairtex/kerrfairtex/wikis/Instalar-KerrFairtex-en-Windows)
- [**Mac**](https://github.com/kerrfairtex/kerrfairtex/-/wikis/How-to-install-KerrFairtex-on-Mac-(macOS,-OS-X)) (en inglés)
- [**cPanel**](https://github.com/kerrfairtex/kerrfairtex/wikis/How-to-install-KerrFairtex-on-cPanel) (en inglés)
- [**Softaculous**](https://github.com/kerrfairtex/kerrfairtex/-/wikis/How-to-install-KerrFairtex-with-Softaculous) (en inglés)
- [**Docker**](https://github.com/kerrfairtex/docker-kerrfairtex/) (en inglés)
...
Descomprima la [última versión](https://www.kerrfairtex.org/es/download/) de KerrFairtex, o clona el repositorio git (y haga el checkout del tag de la [última versión](https://github.com/kerrfairtex/kerrfairtex/-/releases)) en un directorio accesible con el navegador. Edita el archivo `config.inc.sample.php` para definir las variables de configuración apropiadas, y renombralo `config.inc.php`.
...
- `$RosarioPath` Camino completo hacia la instalación de KerrFairtex.
...
- `define( 'ROSARIO_POST_MAX_SIZE_LIMIT', 16 * 1024 * 1024 );` Limitar el tamaño de `$_POST` (16MB por defecto). Detalles [acá](https://github.com/kerrfairtex/kerrfairtex/-/blob/mobile/Warehouse.php#L322).
...
Ahora, está listo para configurar la base de datos de KerrFairtex. Si tiene acceso al símbolo del sistema de su servidor, abra una ventana del terminal, y sigua estas instrucciones.
...
```
2. Crea el usuario kerrfairtex:
```bash
postgres=# CREATE USER kerrfairtex_user WITH PASSWORD 'kerrfairtex_user_password';
```
3. Crea la base de datos kerrfairtex:
```bash
postgres=# CREATE DATABASE kerrfairtex_db WITH ENCODING 'UTF8' OWNER kerrfairtex_user;
```
...
```
3. Crea el usuario kerrfairtex:
```bash
mysql> CREATE USER 'kerrfairtex_user'@'localhost' IDENTIFIED BY 'kerrfairtex_user_password';
```
4. Crea la base de datos kerrfairtex:
```bash
mysql> CREATE DATABASE kerrfairtex_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci;
mysql> GRANT ALL PRIVILEGES ON kerrfairtex_db.* TO 'kerrfairtex_user'@'localhost';
```
5. Salga de MySQL:
```bash
mysql> \q
...
Instrucciones de instalación para Ubuntu. Activar la función PHP `mail()`:
```bash
...
[Guía de Configuración Rápida](https://www.kerrfairtex.org/es/quick-setup-guide/)

[Asegurar KerrFairtex](https://github.com/kerrfairtex/kerrfairtex/-/wikis/Secure-KerrFairtex)
