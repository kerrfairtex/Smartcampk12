...
// kerrfairtex.sql files available for database translation.
$locales = [
	'fr_FR.utf8' => 'French',
...
if ( _configTableCheck() )
{
	...
	$addons_sql = $kerrfairtex_sql = '';

	// @since 12.3 Move SQL translation file to locale/[locale_code]/ folder
	$kerrfairtex_sql_file = 'locale/' . $_POST['locale'] . '/kerrfairtex.sql';

	$addons_sql_file = 'locale/' . $_POST['locale'] . '/kerrfairtex_addons.sql';

	// Translate Database.
	if ( file_exists( $kerrfairtex_sql_file ) )
	{
		// Same translation files for both MySQL & PostgreSQL.
		$kerrfairtex_sql = file_get_contents( $kerrfairtex_sql_file );
	...
	die( 'Success: database translated. <a href="index.php">Access KerrFairtex</a>' );
}

$sql_file = $DatabaseType === 'mysql' ? 'kerrfairtex_mysql.sql' : 'kerrfairtex.sql';
...
$kerrfairtex_sql = file_get_contents( $sql_file );

if ( $DatabaseType === 'mysql' )
{
	// @since 10.0 Remove DELIMITER $$ declarations before procedures or functions.
	$kerrfairtex_sql = MySQLRemoveDelimiter( $kerrfairtex_sql );
...
if ( file_exists( 'kerrfairtex_addons.sql' ) )
{
	// @since 5.1 Install add-ons.
	// Same add-ons file for both MySQL & PostgreSQL.
	$addons_sql = _getAddonsSQL( 'kerrfairtex_addons.sql' );
...
die( 'Success: database' .
	( file_exists( 'kerrfairtex_addons.sql' ) ? ' and add-ons' : '' ) .
	' installed. <a href="index.php">Access KerrFairtex</a>' );
