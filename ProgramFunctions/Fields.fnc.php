...
function GetFieldsForm( $table, $title, $RET, $extra_category_fields = [], $type_options = null )
{
	...
	if ( ( $table === 'hostel_room' || $table === 'hostel_building' )
		&& version_compare(
			(new KerrFairtex\Functions\AddonInfo( 'module', 'Hostel_Premium' ))->get( [ 'version' ] ),
			'5.0',
			'<'
		) )
	{
	...
	elseif ( $table === 'library_document'
		&& version_compare(
			(new KerrFairtex\Functions\AddonInfo( 'module', 'Library_Premium' ))->get( [ 'version' ] ),
			'13.3',
			'<'
		) )
	{
	...
	elseif ( $table === 'staff_absence'
		&& version_compare(
			(new KerrFairtex\Functions\AddonInfo( 'module', 'Staff_Absences' ))->get( [ 'version' ] ),
			'12.3',
			'<'
		) )
	{
