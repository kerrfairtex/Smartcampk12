...
if ( ! function_exists( 'FirstLoginPoll' ) )
{
	 ...
	 *
	 * @return string Poll HTML array or empty string if not 'admin' user or if kerrfairtex.org not reachable.
	 */
	function FirstLoginPoll()
	{
		...
		if ( ! empty( $_REQUEST['poll'] ) )
		{
			...
			// POST poll data to kerrfairtex.org.
			try
			{
				$curl = new curl;

				$curl->post( 'https://www.kerrfairtex.org/installation-poll/poll-submit.php', $data );
			}
			catch ( Exception $e )
			{
		...
		$has_connection = @file_get_contents( 'https://www.kerrfairtex.org/installation-poll/poll-submit.php' );
		...
		$buttons .= ' <input type="button" value="' . AttrEscape( _( 'Cancel' ) ) .
			// @since KerrFairtex 12.5 CSP remove unsafe-inline Javascript
		...
		$fields[] = sprintf(
			...
			URLEscape( 'https://www.kerrfairtex.org/' . $url_lang . 'installation-poll/' )
		);
