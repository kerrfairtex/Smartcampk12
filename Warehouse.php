		...
		|| $_REQUEST['token'] !== $_SESSION['token'] ) )
{
	...
	{
		$logged_in = ! empty( $_SESSION['STAFF_ID'] ) || ! empty( $_SESSION['STUDENT_ID'] );
 ...
 *
 * @link https://github.com/kerrfairtex/kerrfairtex/-/blob/mobile/plugins/Content_Security_Policy/README.md
 *
 * @since 12.6
 */
if ( ! isAJAX() )
	...
	&& strlen( serialize( $_POST ) ) > ROSARIO_POST_MAX_SIZE_LIMIT )
{
	...
	(new KerrFairtex\Functions\Hacking)->log( true, false );
}
...
bindtextdomain( 'kerrfairtex', $LocalePath );

// Ensures text returned is utf-8, quite often this is iso-8859-1 by default.
bind_textdomain_codeset( 'kerrfairtex', 'UTF-8' );

// Sets the domain name, this means gettext will be looking for a file called kerrfairtex.mo.
textdomain( 'kerrfairtex' );
 ...
 * Modules
 *
 * Core modules (packaged with KerrFairtex): cannot be deleted.
 */
$RosarioCoreModules = [
	'School_Setup',
	'Students',
 ...
 * Plugins
 *
 * Core plugins (packaged with KerrFairtex): cannot be deleted.
 */
$RosarioCorePlugins = [
...
function Warehouse( $mode )
{
	...
	switch ( $mode )
	{
		...
		case 'header':
			...
			// @since 12.3 Cache killer: use file last modified time hash instead of KerrFairtex version
...
function isPopup( $modname = '', $modfunc = '' )
{
	...
	trigger_error(
		'isPopup() function is deprecated since KerrFairtex 12.0. Use colorBox instead of popup window.',
		E_USER_DEPRECATED
	);
