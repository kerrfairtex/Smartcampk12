...
{
	$error[] = 'KerrFairtex requires PHP 5.5.9 to run, your version is : ' . PHP_VERSION;
}
...
{
	$error[] = 'The diagnostic.php file needs to be in the KerrFairtex directory to be able to run. Please move it there, and run it again.';
}
...
else
{
	...
	{
		$error[] = 'The value for $RosarioPath in the config.inc.php file is not correct or else the functions directory does not have the correct permissions to be read by the webserver. Make sure $RosarioPath points to the KerrFairtex installation directory and that it is readable by the `' . $_SERVER['USER'] . '` user.';
	}
	...
	{
		$error[] = 'PHP extensions: KerrFairtex relies on the pgsql extension (used to connect to the PostgreSQL database). Please install and activate it.';
	}
	elseif ( $DatabaseType === 'mysql'
		&& ! function_exists( 'mysqli_connect' ) )
	{
		$error[] = 'PHP extensions: KerrFairtex relies on the mysql (or mysqli) extension (used to connect to the MySQL database). Please install and activate it.';
	}
	else
	{
		...
		if ( ! $db_connection )
		{
			$error[] = 'KerrFairtex cannot connect to the ' . ( $DatabaseType === 'mysql' ? 'MySQL' : 'PostgreSQL' ) . ' database server. Please review the database configuration variables in the config.inc.php file.';
...
{
	$warning[] = 'PHP extensions: KerrFairtex relies on the gd extension (used to resize and compress images). Please install and activate it.';
}

// Check for zip extension.
if ( ! extension_loaded( 'zip' ) )
{
	$warning[] = 'PHP extensions: KerrFairtex relies on the zip extension (used to upload add-ons). Please install and activate it.';
}

// Check for curl extension.
if ( ! extension_loaded( 'curl' ) )
{
	$warning[] = 'PHP extensions: KerrFairtex relies on the curl extension (used to make external API calls). Please install and activate it.';
}

// Check for intl extension.
if ( ! extension_loaded( 'intl' ) )
{
	$warning[] = 'PHP extensions: KerrFairtex relies on the intl extension (used for internationalization). Please install and activate it.';
}
...
{
	$warning[] = 'PHP extensions: KerrFairtex relies on the gettext extension (used for translations). Please install and activate it.';
}
...
if ( ! count( $error ) )
{
	echo '<h3>Your KerrFairtex installation is properly configured.</h3>';
}
