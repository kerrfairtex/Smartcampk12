...
function MarkDownInputPreview( $input_id )
{
	...
	<div class="md-preview">
		...
		<a href="https://github.com/kerrfairtex/kerrfairtex/wikis/Markdown-Cheatsheet" title="<?php echo AttrEscape( _( 'Mastering MarkDown' ) ); ?>" target="_blank" class="md-link">
...
function ChosenSelectInput( $value, $name, $title = '', $options = [], $allow_na = 'N/A', $extra = '', $div = true )
{
	...
	trigger_error(
		'ChosenSelectInput() function is deprecated since KerrFairtex 10.7. Use Select2Input() instead.',
		E_USER_DEPRECATED
	);
...
function RadioInput( $value, $name, $title, $options, $allow_na = 'N/A', $extra = '', $div = true )
{
	...
	$display_val = ! isset( $options[ (string) $value ] ) ? '' :
		( is_array( $options[ $value ] ) ? $options[ $value ][1] : $options[ $value ] );
