...
elseif ( isset( $_REQUEST['email_column'] ) )
{
	$email_column = $_SESSION['email_column'] = $_REQUEST['email_column'];
}
	...
	if ( ! empty( $_REQUEST['student'] ) )
	{
		...
		foreach ( (array) $RET as $email => $students )
		{
			...
			if ( ! empty( $_REQUEST['modfunc_send_email_notification'] ) )
			{
				...
				$result = (new KerrFairtex\Functions\Email)->send( $to, $subject[$account], $msg, $reply_to );
			}
