	...
	switch ( $table )
	{
		...
		case 'staff':
			...
			if ( $DatabaseType === 'mysql' )
			{
				...
				s.PROFILE=s2.PROFILE,s.LAST_LOGIN=s2.LAST_LOGIN,s.SCHOOLS=s2.SCHOOLS,
			...
			else
			{
				...
				PROFILE=s2.PROFILE,LAST_LOGIN=s2.LAST_LOGIN,SCHOOLS=s2.SCHOOLS,
		...
		case 'school_periods':
			...
			DBQuery( "INSERT INTO school_periods (SYEAR,SCHOOL_ID,SORT_ORDER,TITLE,
				SHORT_NAME,LENGTH,ATTENDANCE,BLOCK,ROLLOVER_ID)
				SELECT SYEAR+1,SCHOOL_ID,SORT_ORDER,TITLE,
				SHORT_NAME,LENGTH,ATTENDANCE,BLOCK,PERIOD_ID FROM school_periods
				WHERE SYEAR='" . UserSyear() . "'
