 ...
 *
 * @since 12.0
 * @since 12.2 Send KerrFairtex version
 *
...
function AddonInstallationStatisticsPost( $type, $addon_dir )
{
	...
	if ( $_REQUEST['modfunc'] === 'installation_statistics_post' )
	{
		...
		try
		{
			...
			$curl->post(
				'https://www.kerrfairtex.org/addon-statistics/installation-submit.php',
				$data
			);
		}
		catch ( Exception $e )
 ...
 * @since 13.0
 *
 * @uses KerrFairtex\Functions\AddonInfo class
 *
 * @param string $type      Add-on type: module|plugin.
 * @param string $addon_dir Add-on directory. For example: 'My_Module'.
 *
...
{
	$addon_info = new KerrFairtex\Functions\AddonInfo( $type, $addon_dir );
